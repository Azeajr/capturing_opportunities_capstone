...
@app.get("/auto_encoder")
async def auto_encoder():
    content = """
<head>
  <title>Upload Images</title>
  <script src="static/js/htmx.min.js"></script>
      ...
      document
        ...
        .addEventListener("htmx:afterSwap", function (evt) {
          ...
          try {
            ...
            var modelApiEndpoint =
              responseJson.data.attributes.modelApiEndpoint;
            var newAction = "/uploads/collection_images" + modelApiEndpoint;
            ...
            if (formToUpdate) {
              ...
              console.log(
                "Form not found with the specified hx-post attribute"
              );
            }
          } catch (error) {
            console.error(
...
<body>
  <form
    hx-encoding="multipart/form-data"
    hx-post="/uploads/training_images/auto_encoder"
    hx-target="#training_output"
    hx-swap="innerHTML"
  >
    <input name="files" type="file" multiple />
...
<head>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function (event) {
      ...
      document.getElementById("collection_output").addEventListener("htmx:afterSwap", function (evt) {
        console.log("htmx:afterSwap event fired");
        try {
          var responseJson = JSON.parse(this.innerText); // Assuming the server response is correctly formatted JSON
          this.innerText = JSON.stringify(responseJson, null, 2);
...
<body>
  <form
    hx-encoding="multipart/form-data"
    hx-post="/uploads/training_images/svm"
    hx-target="#training_output"
    hx-swap="innerHTML"
  >
    <input name="files" type="file" multiple />
